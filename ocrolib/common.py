...
def write_image_gray(fname,image,normalize=0,verbose=0):
    """If the image is of floating poin type, its values are clipped to the range [0,1],
    multiplied by 255 and converted to unsigned bytes.  Otherwise, the image must be of type unsigned byte."""
    if verbose:
        print("# writing", fname)
